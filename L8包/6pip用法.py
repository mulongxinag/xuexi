...
# 包管理工具：包别人写好的代码。经常有这种情况，比如爬虫框架功能的A包，里面引用了负责解析网页的B包，B包引用了更加基础底层的C包。包关系成树状引用。B包依赖C包。直接使用A包，运行报错缺少B包，缺少依赖包。
...
pip install  包名    #（常用）安装包。安装包的本质是从pypi下载，解压复制C:\Python36\Lib\site-packages 下。默认安装包的最新版本。
...
"""
批量备份和安装
pip freeze > requirement.txt    #将解释器中的包和版本导出到一个文件中
pip install requirent.txt    #根据requirent.txt的信息批量安装对应版本的包。
"""
...
写入下面两行
[global]
index-uri = http://pypi.douban.com/simple/
...
以下不影响：
1.最后黄色警告。pip版本升级提示，可以忽略。
2.黄色警告。pip所在的目录没有添加的环境变量中。
3.requirement already satisfied 。之前已经成功安装过此包了。
4.cache  缓存。之前已经下载过安装包，再次安装不会再从网上下载。
"""
