...
print(os.getcwd())
# 7> 获取当前文件所在文件夹。与os.getcwd()不一样的地方，getcwd会受os.chdir().os.path.dirname只返回当前文件的所在目录，路径分隔用的的正斜杠。__file__表示文件名、自身。
print('7', os.path.dirname(__file__))

# 相对路径和绝对路径（针对资源管理器路径）
# 绝对路径：从盘符或项目根目录写出每一层目录层级到文件。D:\PycharmProjects\tutoial\L8包\3os.py 。优点是准确，缺点是写起来麻烦。
# 相对路径：  .当前目录    ..表示父目录
# 相对3os.py而言，./包引用.py      L8包/1包引用.py     ../L7本地文件读写/xx.py
...
## 作业：使用绝对路径打开L7/chinese_utf-8.txt
with open('chinese_utf-8.txt','r') as f:
    content = f.read()
    print(content)

"""
作业二：
选择资源管理器上的文件夹，批量重命名里面杂乱的文件，文件1,文件2,文件3。
追加1：不同类型文件txt doc jpg .
追加2：读取文件元信息，把创建时间读出来重命名文件。
追加3：修改文件元信息
知识点：for循环  os.listdir  百度os包获取文件名后缀  split('.') 百度os包获取文件
