# 包引用
"""
包 package：组织各个py脚本文件和其他配置、静态文件等，项目由包组成的，确定项目组织的层次结构。说白了就是写好的代码封装起来。
...
__init__.py:包类似windows资源管理器上的文件夹。只是这个文件夹下有一个__init__.py 文件。
一个包下必须有__init__.py。
__init__.py作用
 1.说明是一个包而不是一个文件夹。
 2.控制包的具体行为，功能比文件夹更佳，相当于升级版文件夹。
...
2.from 包名.py脚本 import  脚本中类，方法，变量   :用什么引什么
简写from 包名.py脚本 import *
3.from 包.子包 import 子包里的py脚本。

查找路径：import包的时候，python解释器首先从项目跟目录下开始查找符合的包名。如果未找到再去 解释器安装目录/Lib、安装目录/Lib/sitepackages 目录下查找内置包或三方包。
"""
